...
var maxProfit = function(prices) {
  ...
  const helper = day => {
    ...
    // sell today or no action today.
    let max = helper(day - 1);
    const sellPrice = prices[day];
    for (let i = day - 1; i >= 0; i--) {
      if (prices[i] < sellPrice) {
        max = Math.max(max, helper(i - 2) + sellPrice - prices[i]);
      }
    }
    cache[day] = max;
    return max;
...
//         max = Math.max(max, (dp[j - 2] || 0) + prices[i] - prices[j]);
//       }
//     }
//     dp[i] = max;
//     // console.log(dp[i]);
